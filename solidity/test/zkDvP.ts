...
describe("DvP flows between fungible and non-fungible tokens based on Zeto with anonymity without encryption or nullifiers", function () {
  ...
  it("Initiating a successful DvP transaction with payment inputs", async function () {
    ...
    await expect(zkDvP.connect(Alice.signer).initiateTrade([utxo1.hash, utxo2.hash], [utxo3.hash, utxo4.hash], ZeroHash, 0, 0, ZeroHash)).fulfilled;
  });

  it("Initiating a successful DvP transaction with asset inputs", async function () {
    const utxo1 = newUTXO(10, Alice);
    const utxo2 = newUTXO(20, Alice);
    await expect(zkDvP.connect(Alice.signer).initiateTrade([0, 0], [0, 0], ZeroHash, utxo1.hash, utxo2.hash, ZeroHash)).fulfilled;
  });
  ...
  it("Initiating a successful DvP transaction with payment inputs and accepting by specifying asset inputs", async function () {
    ...
    const tx1 = await zkDvP.connect(Alice.signer).initiateTrade([_utxo1.hash, 0], [utxo2.hash, 0], hash1, 0, 0, ZeroHash);
    ...
    const tx2 = await zkDvP.connect(Alice.signer).completeTrade(tradeId, proof1.encodedProof);
    const tx2Result = await tx2.wait();

    // 4. Bob sends his proof to complete the trade (the trade will be completed)
    const tx3 = await zkDvP.connect(Bob.signer).completeTrade(tradeId, proof2.encodedProof);
  ...
  describe("failure cases", function () {
    ...
    it("Initiating a DvP transaction without payment input or asset input should fail", async function () {
      await expect(zkDvP.connect(Alice.signer).initiateTrade([0, 0], [0, 0], ZeroHash, 0, 0, ZeroHash)).rejectedWith("Payment inputs and asset input cannot be zero at the same time");
    });

    it("Initiating a DvP transaction with payment input but no payment output should fail", async function () {
      const utxo1 = newUTXO(10, Alice);
      const utxo2 = newUTXO(20, Alice);
      await expect(zkDvP.connect(Alice.signer).initiateTrade([utxo1.hash, utxo2.hash], [0, 0], ZeroHash, 0, 0, ZeroHash)).rejectedWith("Payment outputs cannot be zero when payment inputs are non-zero");
    });
    ...
    it("Initiating a DvP transaction with payment inputs and asset inputs should fail", async function () {
      ...
      const utxo3 = newUTXO(25, Bob);
      const utxo4 = newUTXO(5, Alice);
      await expect(zkDvP.connect(Alice.signer).initiateTrade([utxo1.hash, utxo2.hash], [utxo3.hash, utxo4.hash], ZeroHash, utxo3.hash, 0, ZeroHash)).rejectedWith("Payment inputs and asset input cannot be provided at the same time");
    });

    it("Initiating a DvP transaction with asset input but no asset output should fail", async function () {
      const utxo1 = newUTXO(10, Alice);
      await expect(zkDvP.connect(Alice.signer).initiateTrade([0, 0], [0, 0], ZeroHash, utxo1.hash, 0, ZeroHash)).rejectedWith("Asset output cannot be zero when asset input is non-zero");
    });

    it("Accepting a trade using an invalid trade ID should fail", async function () {
      await expect(zkDvP.connect(Bob.signer).acceptTrade(1000, [0, 0], [0, 0], ZeroHash, 0, 0, ZeroHash)).rejectedWith("Trade does not exist");
    });
    ...
    it("Failing cases for accepting a trade with payment terms", async function () {
      ...
      const tx1 = await zkDvP.connect(Alice.signer).initiateTrade([utxo1.hash, 0], [utxo2.hash, 0], mockProofHash, 0, 0, mockProofHash);
      ...
      await expect(zkDvP.connect(Bob.signer).acceptTrade(tradeId, [utxo1.hash, 0], [0, 0], mockProofHash, 0, 0, mockProofHash)).rejectedWith("Payment inputs already provided by the trade initiator");
      await expect(zkDvP.connect(Bob.signer).acceptTrade(tradeId, [0, 0], [utxo2.hash, 0], mockProofHash, 0, 0, mockProofHash)).rejectedWith("Payment outputs already provided by the trade initiator");
      await expect(zkDvP.connect(Bob.signer).acceptTrade(tradeId, [0, 0], [0, 0], mockProofHash, 0, 0, mockProofHash)).rejectedWith("Asset input must be provided to accept the trade");
      await expect(zkDvP.connect(Bob.signer).acceptTrade(tradeId, [0, 0], [0, 0], mockProofHash, utxo3.hash, 0, mockProofHash)).rejectedWith("Asset output must be provided to accept the trade");
    });
    ...
    it("Failing cases for accepting a trade with asset terms", async function () {
      ...
      const tx1 = await zkDvP.connect(Alice.signer).initiateTrade([0, 0], [0, 0], ZeroHash, utxo1.hash, utxo2.hash, mockProofHash);
      const result = await tx1.wait();
      const event = zkDvP.interface.parseLog(result.logs[0]);
      const tradeId = event.args.tradeId;
      ...
      await expect(zkDvP.connect(Bob.signer).acceptTrade(tradeId, [utxo3.hash, utxo4.hash], [utxo5.hash, utxo6.hash], mockProofHash, utxo1.hash, utxo2.hash, mockProofHash)).rejectedWith("Asset inputs already provided by the trade initiator");
      await expect(zkDvP.connect(Bob.signer).acceptTrade(tradeId, [utxo3.hash, utxo4.hash], [utxo5.hash, utxo6.hash], mockProofHash, 0, utxo2.hash, mockProofHash)).rejectedWith("Asset outputs already provided by the trade initiator");
