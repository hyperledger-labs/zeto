...
describe("Zeto based fungible token with anonymity without encryption or nullifier", function () {
  ...
  describe("lock() tests", function () {
    ...
    it("the designated delegate can use the proper proof to spend the locked state", async function () {
      ...
      await expect(sendTx(Charlie, inputCommitments, outputCommitments, encodedProof)).to.be.fulfilled;
    });
    ...
    it("unlocks a previously locked UTXO, and spend as usual", async function () {
      ...
      await expect(zeto
        .connect(Alice.signer)
        .delegateLock([lockedUtxo2.hash as BigNumberish], "0x0000000000000000000000000000000000000000", "0x")).to.be.fulfilled;

      // now Bob as the owner can spend the UTXO as usual
      await expect(doTransfer(Bob, [lockedUtxo2, ZERO_UTXO], [utxo1, ZERO_UTXO], [Charlie, Charlie])).to.be.fulfilled;
    });
  });
