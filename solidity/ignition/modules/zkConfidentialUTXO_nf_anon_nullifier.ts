...
const VerifierModule = buildModule("Groth16Verifier_NF_Anonymity_Nullifier", (m) => {
  const verifier = m.contract('Groth16Verifier_NF_Anonymity_Nullifier', []);
  return { verifier };
});

export default buildModule("zkConfidentialUTXO_NF_Anonymity_Nullifier", (m) => {
  ...
  const zkConfidentialUTXO = m.contract('zkConfidentialUTXO_NF_Anonymity_Nullifier', [verifier, registry], {
    libraries: {
      SmtLib: smtLib,
      PoseidonUnit3L: poseidon3,
      Commonlib: commonlib,
