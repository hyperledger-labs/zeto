...
// Convert a 256-bit number to a 254-bit number by removing the most significant two bits
function convert256To254(hex256bits) {
  // pad the hex string to 64 characters (256 bits)
  const buff = Buffer.from(hex256bits.padStart(64, '0'), 'hex');
  // disgard the first 2 bits
  const bits3To8 = buff[0].toString(2).padStart(8, '0').slice(2);
  // prepend with 0s to make it 8 bits and convert to hex
  const newHex = parseInt('00' + bits3To8, 2).toString(16);
  // replace the first byte with the new hex value
  buff[0] = Buffer.from(newHex, 'hex')[0];
