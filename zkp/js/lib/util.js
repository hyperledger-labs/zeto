...
function buildPlaintext(inputValues, inputSalts, outputValues, outputSalts, senderPubKey, receiversPubKeys) {
  return JSON.stringify([
    inputValues,
    inputSalts.map((x) => x.toString(16)),
    senderPubKey.toString(16),
    outputValues.map((x) => x.toString(16)),
    outputSalts.map((x) => x.toString(16)),
    receiversPubKeys.map((x) => x.toString(16)),
  ]);
}
